...
(function (jQuery) {
    ...
    function addVtHeader(div, data, cellHeight) {
      ...
      headerDiv.append(
        jQuery("<div>", {
          ...
          $("<div class='search-form-group'><input type='text' placeholder='Search Resource'> <a href='javascript:void(0);' class='search-cta'></a></div>"),
          // $("")
        )
      )
      for (var i = 0; i < data.length; i++) {
        ...
        itemDiv.append(
          jQuery("<div>", {
            ...
            $("<span class='emp-name'>" + data[i].name + "</span>"),
            $(
              "<span class='utilisation-perc'>" +
                data[i].utilisationPercantage +
                "%</span>"
            ),
            $("<a>", { class: "insert-record" }),
            $(
              "<div class='department-name'><span>Dept: </span>" +
                data[i].departmentName +
                "</div>"
            ),
            $(
              "<div class='reporting-manager'><span>RM: </span>" +
                data[i].reportingManager +
                "</div>"
            ),
            $(
              "<div class='project-count'><span>Project count: </span>" +
              data[i].projectCount +
              "</div>"
            )
          )
        ...
        for (var j = 0; j < data[i].series.length; j++) {
          if (j > 2) {
            seriesDiv.append(
              jQuery("<div>", {
                class: "d-none ganttview-vtheader-series-name",
      ...
      for (var i = 0; i < data.length; i++) {
        for (var j = 0; j < data[i].series.length; j++) {
          if (j > 2) {
            blocksDiv.append(
              jQuery("<div>", {
                class: "d-none ganttview-block-container",
                "data-name": data[i].name,
              })
            );
          } else {
      ...
      for (var i = 0; i < data.length; i++) {
        for (var j = 0; j < data[i].series.length; j++) {
          ...
          var block = jQuery("<div>", {
            ...
            "data-sdate": data[i].series[j].start.toString("d/M/yyyy"),
            "data-edate": data[i].series[j].end.toString("d/M/yyyy"),
            "data-type": data[i].series[j].bookingType
          });
          ...
          block.append(
            ...
            jQuery("<div>", { class: "ganttview-block-text" }).text('Booking: '+series.bookingType)
          );
  ...
  var Behavior = function (div, opts) {
    ...
    function updateDataAndPosition(div, block, cellWidth, startDate) {
      ...
      block.attr('data-sdate', newStart.toString("d/M/yyyy"));
      block.attr('data-edate', block.data("block-data").end.toString("d/M/yyyy"));
  ...
  $(".view-more-link").live("click", function () {
    if ($(this).attr("data-view") == "more") {
      ...
      function showViewMoreProject(data, resourceName) {
        var $data = data;
        ...
        $data.each(function () {
          if (
            $(this).attr("data-name") &&
            $(this).attr("data-name") == $resourceName
          ) {
            $(this).removeClass("d-none");
          }
        });
      }
      $(this).text("View-Less").attr("data-view", "less");
    } else {
      showViewLessProject(
        $(".ganttview-block-container"),
        $(this).attr("data-name")
      ...
      function showViewLessProject(data, resourceName) {
        var $data = data;
        var $resourceName = resourceName;
        $data.each(function () {
          if (
  ...
  // disbled to edit datefield

  $('input[type=date]').live('input',function(e) {
    e.preventDefault();
  })
})(jQuery);
